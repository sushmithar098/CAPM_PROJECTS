...
entity Person {
    ...
    @mandatory
    @assert.range: '1..1'
        passport : Association to one Passport;
}

entity Passport {
    key ID     : UUID;
        @assert.pattern: '[A-Z]{2}[0-9]{7}'
        number : String;
...
entity Student {
    key StudentID : Integer;
        ...
        enroll   : Association to many Enrollments on enroll. students=$self;
        
}
entity Course {
    key CourseID : Integer;
        title     : String;
        enroll  : Association to many Enrollments  on enroll.courses=$self;
...
entity Author {
  ...
  books : Composition of many AuthorBooks on books.author=$self;
}
